...
test("exact", (t) => {
  ...
  function assertFails(tt, validator, element, propName, componentName) {
    tt.equal(
      callValidator(validator, element, propName, componentName) instanceof
        Error,
      true
    );
  }
  ...
  t.test("adds one extra key", (st) => {
    ...
    const propTypesOwnKeys = ownKeys(propTypes)
      .concat(specialProperty)
      .sort(stringSort);
  ...
  t.test("exact()", (st) => {
    ...
    st.test(
      "passes via normal propTypes when given only known props",
      (s2t) => {
        const { Component } = setup();
